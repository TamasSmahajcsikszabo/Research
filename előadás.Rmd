...
tidied_plot <- tidied %>% 
  ...
  gather(5:10,
         key = delay,
         value = delay_degree
  )%>% rename(RT_original= RT)

thresholded_plot <- thresholded %>% 
  select(1:4) %>% 
  mutate(
    delay1 = 0,
    delay2 = 50,
  ...
  gather(5:10,
         key = delay,
         value = delay_degree
  )%>% rename(RT_thresholded = RT)

SD_filtered_plot <- SD_filtered %>% 
  select(1:4) %>% 
  mutate(
    delay1 = 0,
    delay2 = 100,
  ...
  gather(5:10,
         key = delay,
         value = delay_degree
  )%>% rename(RT_SD_filtered = RT)

winsorized_plot <- winsorized %>% 
  mutate(
    delay1 = 0,
    delay2 = 150,
    delay3 = 300,
  ...
  gather(5:10,
         key = delay,
         value = delay_degree
  ) %>% rename(RT_winsorized = RT)

all_data_plot <- bind_rows(tidied_plot, SD_filtered_plot, thresholded_plot, winsorized_plot)

plot_ly(alpha = 0.6,frame = ~all_data_plot$delay) %>%
  add_histogram(x = ~all_data_plot$RT_original + all_data_plot$delay_degree, name = "eredeti adatok") %>% 
  add_histogram(x = ~all_data_plot$RT_SD_filtered + all_data_plot$delay_degree, name = "3 standard szórással szűrt adatok") %>%
  add_histogram(x = ~all_data_plot$RT_thresholded + all_data_plot$delay_degree, name = "küszöbértékekkel szűrt adatok") %>% 
  add_histogram(x = ~all_data_plot$RT_winsorized + all_data_plot$delay_degree, name = "winsorizált adatok") %>%
